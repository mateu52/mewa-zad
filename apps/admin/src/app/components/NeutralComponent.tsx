...
export const NeutralComponent: React.FC<PositiveComponentProps> = ({ data }) => {
    ...
    return(
        <div>
            <div className="p-2">
                <h1>ostatnie 3 NEUTRALNE komentarze:</h1>
                {latestThreeReviews.map((elem: Review) => (
                    <p className="bg-gray-200 pl-2 mb-1 break-words w-64" key={elem.id}>{elem.fields.content}</p>
                ))}
            </div>
        </div>
    )
